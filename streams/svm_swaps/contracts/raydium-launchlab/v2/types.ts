...
export type AmmCreatorFeeOn_QuoteToken = undefined;

export const AmmCreatorFeeOn_QuoteToken = unit;

export type AmmCreatorFeeOn_BothToken = undefined;

export const AmmCreatorFeeOn_BothToken = unit;

/**
 * migrate to cpmm, creator fee on quote token or both token
 */
export type AmmCreatorFeeOn =
  | {
      kind: 'QuoteToken';
      value?: AmmCreatorFeeOn_QuoteToken;
    }
  | {
      kind: 'BothToken';
      value?: AmmCreatorFeeOn_BothToken;
    };

/**
 * migrate to cpmm, creator fee on quote token or both token
 */
export const AmmCreatorFeeOn: Codec<AmmCreatorFeeOn> = sum(1, {
  QuoteToken: {
    discriminator: 0,
    value: AmmCreatorFeeOn_QuoteToken,
  },
...
export interface BondingCurveParam {
  migrateType: number;
...
export const BondingCurveParam: Codec<BondingCurveParam> = struct({
  migrateType: u8,
  migrateCpmmFeeOn: u8,
  supply: u64,
  totalBaseSell: u64,
...
export interface PlatformConfig {
  ...
  curveParams: Array<PlatformCurveParam>;
}
...
export const PlatformConfig: Codec<PlatformConfig> = struct({
  ...
  creatorFeeRate: u64,
  transferFeeExtensionAuth: address,
  padding: fixedArray(u8, 180),
  curveParams: array(ref(() => PlatformCurveParam)),
});
...
export const PlatformConfigInfo: Codec<PlatformConfigInfo> = struct({
  feeWallet: address,
  nftWallet: address,
...
export type PlatformConfigParam_CpSwapConfig = undefined;

export const PlatformConfigParam_CpSwapConfig = unit;

export type PlatformConfigParam_AllInfo = [PlatformConfigInfo];

export const PlatformConfigParam_AllInfo = tuple([ref(() => PlatformConfigInfo)]);
...
export type PlatformConfigParam =
  ...
  | {
      kind: 'CpSwapConfig';
      value?: PlatformConfigParam_CpSwapConfig;
    }
  | {
      kind: 'AllInfo';
      value: PlatformConfigParam_AllInfo;
    };
...
export interface PlatformCurveParam {
  epoch: bigint;
  index: number;
  globalConfig: string;
...
export const PlatformCurveParam: Codec<PlatformCurveParam> = struct({
  epoch: u64,
  index: u8,
  globalConfig: address,
  bondingCurveParam: ref(() => BondingCurveParam),
  padding: fixedArray(u64, 50),
});
...
export const PoolState: Codec<PoolState> = struct({
  ...
  creator: address,
  tokenProgramFlag: u8,
  ammCreatorFeeOn: ref(() => AmmCreatorFeeOn),
...
export interface TransferFeeExtensionParams {
  transferFeeBasisPoints: number;
  maximumFee: bigint;
}

export const TransferFeeExtensionParams: Codec<TransferFeeExtensionParams> = struct({
